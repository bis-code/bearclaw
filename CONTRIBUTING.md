...
2. **Every hook ships a test.** A hook at `hooks/foo.sh` needs `hooks/foo.test.sh`
...
3. **Hooks are POSIX `sh`, not bash.** They run on every session start; keep them
   under ~50ms and make them fail open — a broken hook must never block work.
4. **Anti-bloat.** If a thing isn't load-bearing for everyday use, it doesn't ship.
   Explain in the PR why you need it, not why it might be nice.
5. **Tests must never mutate real machine state.** This bit us: an early test
