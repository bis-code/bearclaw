...
# Root cause of the 2026-06-08 projects-root wipe was a background agent running with
...
# (read-only agents, or accepted risk). See rules/background-agent-safety.md.
...
if [ "$BG" = "true" ] && [ "$ISO" != "worktree" ]; then
  ...
  if [ ! -f "$IMARK" ]; then
    ...
    jq -n --arg extra "$MODEL_NOTE" \
      '{hookSpecificOutput:{hookEventName:"PreToolUse",permissionDecision:"deny",permissionDecisionReason:("Background dispatch without worktree isolation. Add isolation:\"worktree\" so a runaway agent works on a throwaway copy, never your live tree — bgIsolation:none on the live projects-root tree caused the 2026-06-08 wipe. If this agent is read-only or you accept the risk, re-issue the same call (denied once per session)." + (if $extra != "" then " " + $extra else "" end))}}'
    exit 0
  fi
fi
...
if [ "$GP" -eq 1 ] && [ -n "$TEXT" ]; then
  ...
  elif m 'security (review|audit)|owasp|vulnerab'; then match=security-reviewer
  ...
  elif m 'dead code|duplicated? (logic|code)|unused (import|export)|obsolete scaffold'; then match=refactor-cleaner
  elif m '\btdd\b|test-?first|failing test first'; then match=tdd-guide
  elif m 'update (the )?(readme|changelog|openapi)|sync (the )?docs'; then match=doc-updater
  elif m 'implementation plan|plan (the |a |this )(feature|implementation|change)|map (the )?affected (code|files)'; then match=planner
  ...
  elif m 'review (the |this |my )?(diff|changes?|pr|code)'; then match=code-reviewer
