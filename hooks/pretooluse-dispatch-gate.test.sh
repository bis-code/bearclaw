...
out=$(run '{"tool_name":"Agent","tool_input":{"subagent_type":"go-reviewer","model":"sonnet","prompt":"review the diff"},"session_id":"s1"}')
...
out=$(run '{"tool_name":"Agent","tool_input":{"subagent_type":"planner","prompt":"plan the feature"},"session_id":"s5"}')
...
out=$(run '{"tool_name":"Agent","tool_input":{"subagent_type":"planner","prompt":"another planning task"},"session_id":"s5"}')
...
printf '%s' "$out" | grep -q 'code-reviewer' && ok "t10 per-suggestion tracking" || bad "t10" "$out"
